...
import { fetchAddHire, fetchHire } from "../listhire/hireSlide";
import { fetchOption } from "../redux/khungcanhSlide";
...
function Lease() {
  ...
  const [listTamthoi, setListtamthoi] = useState([]);
  const [dataLease, setDatalease] = useState(obj);

  function ShowInfoToast() {
    toast({
      title: "Thông báo",
      message: "Căn hội của bạn đã được đăng trên Airbnb",
      type: "info",
      duration: 4000,
    });
  }
  ...
  useEffect(() => {
    ...
    if(checkBox[1] == "Khung cảnh tuyệt vời"){
      dispatch(fetchOption("khungcanh"))
    }
    if(checkBox[1] == "Bãi biển"){
      dispatch(fetchOption("baibien"))
    }
    if(checkBox[1] == "Lâu đàu"){
      dispatch(fetchOption("laudai"))
    }
  }, [checkBox[1]]);

  useEffect(() => {
    return () => {
      for (let i = 0; i < listTamthoi.length; i++) {
        URL.revokeObjectURL(listTamthoi[i]);
      }
    };
  }, [listTamthoi]);

  function uploadI(img) {
    let listko = [];
    let array = [...img];
    if (array.length === 5) {
      for (let i = 0; i < array.length; i++) {
        listko.push(URL.createObjectURL(array[i]));

        setListtamthoi([...listko]);
        setImageUpload([...img]);
      }
    }
  }
  ...
  function handleSubmit() {
    let arrayImg = ["", "", "", "", ""];
    let array = [...imageUpload];

    for (let i = 0; i < array.length; i++) {
      const imageRef = ref(storage, `image/${array[i].name}`);
      const uploadTask = uploadBytesResumable(imageRef, array[i]);
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            ...
            if (
              ...
              setDatalease({ ...dataLease, img: arrayImg });
            }
          });
        }
      );

      uploadBytes(imageRef, array[i]).then((e) => {});
    }
  }
 ...
 let listKhungcanh = useSelector((state) => state.khungcanhList)
  ...
  useEffect(() => {
    if (dataLease.img.length == 5 && dataLease.img[0] != "") {
        ...
        "listHire": [
          ...listHire.listHire.listHire,
          {
            ...dataLease,
            idsp:listKhungcanh.khungcanh.length,
            id:listHire.listHire.listHire.length
          }
  ...
  }, [dataLease.img.length]);

  return (
    <>
      <Header></Header>
      <div className="tablist">
        <div className="tableSell">
          <div className="discrip" style={{ marginBottom: "10px" }}>
            <div className="discrip text">Cho thuê nhà ở thông qua</div>
          ...
          <input
            onChange={(e) => {
              setDatalease({ ...dataLease, tenchunha: `${e.target.value}` });
            }}
            type="text"
            id="nameBoss"
            placeholder="Tên chủ nhà"
          />
          <span style={{ visibility: "hidden" }} className="validate">
            Không để trống
          </span>
          <input
            onChange={(e) => {
              setDatalease({ ...dataLease, datnuoc: `${e.target.value}` });
            }}
            type="text"
            id="nameCountry"
          ...
          <input
            onChange={(e) => {
              setDatalease({ ...dataLease, tencanho: `${e.target.value}` });
            }}
            type="text"
            id="nameHouse"
          ...
          <input
            onChange={(e) => {
              setDatalease({ ...dataLease, diachi: `${e.target.value}` });
            }}
            type="text"
            id="price"
            placeholder="Địa chỉ cụ của khu vực"
          />
          <span style={{ visibility: "hidden" }} className="validate">
            Không để trống
          </span>
          <input
            onChange={(e) => {
              setDatalease({ ...dataLease, price: `${e.target.value}` });
            }}
            type="number"
            id="price"
          ...
          <div
            style={{ position: "relative", width: "99%", marginRight: "10px" }}
          >
            <input
              type="text"
              id="max-user"
            ...
            ></i>
            <div
              className="btl-checkBox"
              style={arrow === true ? { display: "" } : { display: "none" }}
            >
              <div className="checkBox" style={{ display: "flex" }}>
                <input
                  onClick={() => setArrow(false)}
                  checked={checkBox[0] === 1}
                  onChange={() => {
                    setChecked([1, "Khung cảnh tuyệt vời"]);
                  }}
                  type="checkbox"
                />
                <label htmlFor="">Khung cảnh tuyệt vời</label>
              </div>
              <div className="checkBox" style={{ display: "flex" }}>
                <input
                  onClick={() => setArrow(false)}
                  checked={checkBox[0] === 2}
                  onChange={() => {
                    setChecked([2, "Bãi biển"]);
                  }}
                  type="checkbox"
                />
          ...
          <div style={{ display: "flex", alignItems: "center" }}>
            <label style={{ width: "100px" }}>Chọn 5 ảnh</label>
            <input
              type="file"
              multiple
              onChange={(e) => uploadI(e.target.files)}
            />
          </div>
          <div style={{ display: "flex", alignItems: "center", width: "100%" }}>
            <div style={{ minWidth: "140px" }}>Thời gian cho thuê từ</div>
            <input
              type="date"
              ...
              onChange={(e) => {
                setDatalease({ ...dataLease, dateStart: `${e.target.value}` });
              }}
            />
            <div style={{ minWidth: "30px" }}> đến</div>
            <input
              type="date"
              min={`${new Date().getFullYear()}-${
                new Date().getMonth() < 10 ? 0 : ""
              ...
              onChange={(e) => {
                setDatalease({ ...dataLease, dateEnd: `${e.target.value}` });
              }}
            />
          </div>
        </div>
        <div className="list-img">
          <div className="img-large">
            <img src={listTamthoi.length > 0 ? listTamthoi[0] : ""} alt="" />
          </div>
          <div className="img-small">
            <img src={listTamthoi.length > 0 ? listTamthoi[1] : ""} alt="" />
            <img src={listTamthoi.length > 0 ? listTamthoi[2] : ""} alt="" />
            <img src={listTamthoi.length > 0 ? listTamthoi[3] : ""} alt="" />
            <img src={listTamthoi.length > 0 ? listTamthoi[4] : ""} alt="" />
          </div>
          <button id="comfirm" onClick={() => handleSubmit()}>
            Xác nhận
          </button>
        </div>
      </div>
